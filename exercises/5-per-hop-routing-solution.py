...
if __name__ == "__main__":
    ...
    node0_entries = [
        ...
        TimeFlowEntry(
            dst=1, arrival_ts=1, hops=[TimeFlowHop(cur_node=0, send_ts=1, send_port=0)]
        ),  # Send to node2
        TimeFlowEntry(
    ...
    node1_entries = [
        ...
        TimeFlowEntry(
            dst=0, arrival_ts=1, hops=[TimeFlowHop(cur_node=1, send_ts=1, send_port=0)]
        ),  # Send to node3
        TimeFlowEntry(
    ...
    node2_entries = [
        TimeFlowEntry(
            dst=1, arrival_ts=1, hops=[TimeFlowHop(cur_node=2, send_ts=2, send_port=0)]
        )  # Forward for 0->1
    ]

    node3_entries = [
        TimeFlowEntry(
            dst=0, arrival_ts=1, hops=[TimeFlowHop(cur_node=3, send_ts=2, send_port=0)]
        )  # Forward for 1->0
    ]
    ...
    net.add_time_flow_entry(node_id=2, entries=node2_entries, routing_mode="Per-hop")
    net.add_time_flow_entry(node_id=3, entries=node3_entries, routing_mode="Per-hop")
