...
class TorSwitchHandler : virtual public TorSwitchIf {
 public:
  ...
  int32_t set_active_queue(const int32_t active_q) {
    bm::Logger::get()->trace("set_active_queue");
    return switch_->set_active_queue(active_q);
  }

  int32_t get_active_queue() {
    bm::Logger::get()->trace("get_active_queue");
    return switch_->get_active_queue();
  }
  ...
  void get_device_metric(tswitch_runtime::MonitorResult& _return) {
