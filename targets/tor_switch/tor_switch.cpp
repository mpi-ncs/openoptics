...
TorSwitch::TorSwitch(
  bool enable_swap, port_t drop_port,
  ...
  : Switch(enable_swap),
    ...
    egress_cq_buffers(1024, nb_time_slices),
    output_buffer(128),
...
size_t
TorSwitch::set_active_queue(size_t qid) {
  if (calendar_queue_mode == TorSwitch::CalendarQueueMode::TIME_BASED) {
    printf("Cannot set active queue at time-based mode.\n");
    return 1;
  }
  //BMLOG_DEBUG("Setting active queue to {}", qid);
  egress_cq_buffers.set_active_queue(qid);
  //BMLOG_DEBUG("Finish setting active queue to {}", qid);
  return 0;
}
size_t
TorSwitch::get_active_queue() const {
  //BMLOG_DEBUG("Getting active queue.");
  size_t qid = egress_cq_buffers.get_active_queue();
  //BMLOG_DEBUG("Finish getting active queue {}.", qid);
  return qid;
}
...
TorSwitch::get_num_queued_packets(std::string& _return) const {
  ...
  for (const auto& [port_id, queue_id, q_size] : egress_cq_buffers.get_port_queue_size()) {
      output += "(" + std::to_string(port_id) + "," + std::to_string(queue_id)
...
void
TorSwitch::get_device_metric(tswitch_runtime::MonitorResult& _return) const {

  tswitch_runtime::MonitorResult monitor_result;

  monitor_result.port_queue_metrics = std::vector<tswitch_runtime::PortQueueMetric>();

  for (const auto& item: egress_cq_buffers.get_port_queue_size()) {
    tswitch_runtime::PortQueueMetric pm;
...
TorSwitch::enqueue_cq(port_t egress_port, size_t send_time_slice, std::unique_ptr<Packet> &&packet) {
  ...
  if (send_time_slice >= nb_time_slices) {
    bm::Logger::get()->error("Sending time slice {} out of range {}, dropping packet", send_time_slice, nb_time_slices);
    return;
  }
  ...
  bool push_flag = egress_cq_buffers.push_front(
      egress_port, send_time_slice,
      std::move(packet));
  // push_front returns false if queue is full
  if (push_flag == false) nb_pkts_dropped++;
}
...
TorSwitch::ingress_thread() {
  ...
  while (1) {
    ...
    if (intermediateForward == 1) {
      ...
      BMLOG_DEBUG_PKT(*packet, "Enqueue to normal buffer port {}.", egress_port);
      enqueue(egress_port, std::move(packet));
    } else {
      // Enqueue to calendar queue.
      size_t send_time_slice = -1; // queue id in TA architecture
      ...
      BMLOG_DEBUG_PKT(*packet, "Enqueue to port {} cq {} out of {}.", egress_port, send_time_slice, nb_time_slices);
      enqueue_cq(egress_port, send_time_slice, std::move(packet));

      BMLOG_DEBUG("Finish enqueue to port {} cq {} out of {}.", egress_port, send_time_slice, nb_time_slices);

      BMLOG_DEBUG("Ingress: Current time {}(ms), active q {}", get_ts().count() / 1000, calculate_active_q());
    }
  }
}
...
size_t
TorSwitch::calculate_active_q() {
  size_t active_q;
  if (calendar_queue_mode == CalendarQueueMode::TIME_BASED) {
    active_q = ts2time_slice(get_ts().count());
  } else {
    active_q = get_active_queue();
  }
  return active_q;
}
...
TorSwitch::egress_cq_thread() {
  ...
  while (1) {
    std::unique_ptr<Packet> packet;
    size_t port;
    int64_t ts_ms = get_ts().count();
    size_t active_q = calculate_active_q();

    BMLOG_DEBUG("Current time {}, active q {}", ts_ms, active_q);

    while(!egress_cq_buffers.pop_back(active_q, &port, &packet)) {
      active_q = calculate_active_q();
    }
