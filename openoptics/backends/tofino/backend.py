...
class TofinoBackend(BackendBase):
    ...
    @staticmethod
    def _load_config(config_file: Optional[str]) -> Optional[dict]:
        """Load and return the TOML config dict, or None if unavailable."""
        if config_file is None:
            logger.warning(
                "No Tofino config_file provided. SSH deployment will be skipped."
            )
            return None
        if not os.path.exists(config_file):
            logger.warning(
                "Tofino config file not found at '%s'. "
                "SSH deployment will be skipped.",
                config_file,
            )
            return None
        try:
