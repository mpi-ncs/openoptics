...
class TorSwitchAPI(runtime_CLI.RuntimeAPI):
    ...
    @handle_bad_input
    def do_set_active_queue(self, line):
        "Set the active queue for all ports: set_active_queue <queue_id>"
        args = line.split()
        self.exactly_n_args(args, 1)
        active_qid = self.parse_int(args[0], "queue_id")
        self.sswitch_client.set_active_queue(active_qid)

    @handle_bad_input
    def do_get_active_queue(self):
        "Get id of active calendar queue."
        print(self.sswitch_client.get_active_queue())
    ...
    @handle_bad_input
    def do_get_device_metric(self, line):
        "Get the device metric"
        print(self.sswitch_client.get_device_metric())
