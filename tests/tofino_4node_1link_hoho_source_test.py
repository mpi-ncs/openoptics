"""End-to-end Tofino testbed test: 4 logical ToRs, 1 uplink, HoHo + source routing.

Same as tofino_4node_1link_hoho_test.py except the routing is deployed with
`routing_mode="Source"`. With source routing the source ToR looks up the full
path in time_flow_table_source; 2-hop paths pack the second hop into
hdr.sr_entry so the transit ToR forwards without another table lookup.

Run from inside the Docker container:
    sudo docker exec -w /openoptics openoptics bash -c \\
        "echo '<jump_host_password>' | python3 tests/tofino_4node_1link_hoho_source_test.py"
"""
import logging
import sys
import time
