"""End-to-end Tofino testbed test: 4 logical ToRs, 2 uplinks each, HoHo routing.

Same as tofino_4node_2link_direct_test.py except for the routing algorithm:
uses OpticalRouting.routing_hoho(max_hop=2) instead of routing_direct.

  * backend     : Tofino
  * topology    : opera(nb_node=4, nb_link=2, guardband=True)
  * routing     : routing_hoho(max_hop=2) (Per-hop mode)
  * slice       : 50 us per time slice
  * arch        : direct
  * config      : openoptics/backends/tofino/config_2tor_2link.toml
                  Each logical ToR has 2 OCS uplinks on the SAME pipe:
                    tor 0 (pipe 1): cages 7/0 + 8/0
                    tor 1 (pipe 2): cages 15/0 + 16/0
                    tor 2 (pipe 3): cages 23/0 + 24/0
                    tor 3 (pipe 0): cages 31/0 + 32/0
...
Verifies the deployment by:
  ...
  4. pinging in both directions between the two servers
     (s13 <-> s12, expected sub-ms RTT, 0% loss)

Run from inside the Docker container:
    sudo docker exec -w /openoptics openoptics bash -c \\
        "echo '<jump_host_password>' | python3 tests/tofino_4node_2link_hoho_test.py"
"""
import logging
import sys
import time
...
# Dump per-port TX/RX via ucli pm show so we can correlate what actually
# traversed the fabric with the generated routing JSON.
print()
print("=== ucli pm show on tor switch (verify which cages carried traffic) ===")
