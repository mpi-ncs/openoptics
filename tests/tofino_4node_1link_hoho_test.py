"""End-to-end Tofino testbed test: 4 logical ToRs, 1 uplink each, HoHo routing.

Same as tofino_4node_1link_direct_test.py except for the routing algorithm:
uses OpticalRouting.routing_hoho(max_hop=2) instead of routing_direct.

HoHo allows packets to wait one hop at an intermediate ToR so that a path
exists for every (src, dst) pair even when no direct circuit is scheduled
between them in the current time slice.
...
Run from inside the Docker container:
    sudo docker exec -w /openoptics openoptics bash -c \\
        "echo '<jump_host_password>' | python3 tests/tofino_4node_1link_hoho_test.py"
"""
import logging
import sys
import time
