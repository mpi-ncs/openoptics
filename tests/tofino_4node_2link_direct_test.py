...
Run from inside the Docker container:
    sudo docker exec -w /openoptics openoptics bash -c \\
        "echo '<jump_host_password>' | python3 tests/tofino_4node_2link_direct_test.py"
"""
import logging
import sys
import time
...
print()
print("=== Ping: ToR 0 -> ToR 1 ===")
dst_ip = net._backend._tor_to_ip.get(1, "10.29.2.12")
try:
    out = net._backend.server_exec(0, "ping -c 5 -W 2 %s 2>&1" % dst_ip)
    print(out)
except Exception as e:
    print("Error: %s" % e)

print("=== Ping: ToR 1 -> ToR 0 ===")
src_ip = net._backend._tor_to_ip.get(0, "10.29.2.13")
try:
    out = net._backend.server_exec(1, "ping -c 5 -W 2 %s 2>&1" % src_ip)
    print(out)
except Exception as e:
    print("Error: %s" % e)

# Verify BOTH uplinks carry traffic by inspecting per-port TX counters via ucli.
print()
print("=== ucli pm show on tor switch (verify both uplinks TX) ===")
