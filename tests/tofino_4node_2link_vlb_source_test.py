"""End-to-end Tofino testbed test: 4 logical ToRs, 2 uplinks, VLB + source routing.

Uses runtime-random VLB with 2-link opera topology.  Random<> picks between
port 0 and port 1 per packet.  The intermediate ToR resolves the final hop
via cal_port_slice_to_node.

Run from inside the Docker container:
    sudo docker exec -w /openoptics openoptics bash -c \\
        "echo '<jump_host_password>' | python3 tests/tofino_4node_2link_vlb_source_test.py"
"""
import logging
import sys
import time
