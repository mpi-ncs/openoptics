...
Now we will try something different, **multi-hop routing**, where packets are forwarded through intermediate nodes when a direct path is not immediately available.
...
But with multi-hop routing, the packet can be forwarded by ToR0 to ToR2 at time slice 1, and then forwarded by ToR2 to ToR1 at time slice 2 — `h1` then receives it from ToR1.
